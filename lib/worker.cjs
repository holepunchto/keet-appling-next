...
const { encode, decode, format, nuke, getError } = require('./utils')
const { Progress } = require('./progress')

console.log(bootstrap.validate)
...
async function install() {
  ...
  try {
    try {
      const valid = await bootstrap.validate(config.dir)
      console.log('VALID', valid, config.dir)
      if (!valid) {
        app.broadcast(encode({ type: 'nuclear' }))
        return
      }
    ...
    app.broadcast(encode({ type: 'error', error: getError(e) }))
  } finally {
    clearInterval(bootstrapInterval)
  }
}
