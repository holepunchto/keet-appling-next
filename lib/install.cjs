...
async function install(id, opts = {}) {
  ...
  if (env.SNAP_USER_COMMON) dir = path.join(env.SNAP_USER_COMMON, 'pear')

  using lock = await preflight(id, 'tmp/pear')
  ...
  function onViewMessage(message) {
    ...
    switch (msg) {
      ...
      case 'reset':
        app.broadcast(encode({ type: 'reset' }))
        view.postMessage({ type: 'state', state: 'installing' })
        break
    }
  }
  ...
  function onWorkerMessage(message) {
    ...
    switch (msg.type) {
      ...
      case 'nuclear':
        console.log('nuclear')
        view.postMessage({ type: 'state', state: 'nuclear' })
        break
      case 'error':
        view.postMessage({ type: 'state', state: 'error', error: msg.error })
        break
    }
  }
