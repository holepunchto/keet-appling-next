...
const RESET_CONFIRMATION = 'RESET APP'
...
module.exports = (opts = {}) => html`
  <style>
    ...
    :root {
      ...
      --color-grey-200: #a0a8b8;
      --color-grey-300: #707785;
      --color-grey-500: #30333f;
      --color-grey-600: #222430;
      --color-grey-700: #1a1d29;
    ...
    .reset-group .button-group {
      margin-top: 0;
    }

    .status:has(.reset-group:not(.hidden)),
    .status:has(.error-message:not(.hidden)) {
      height: auto;
    }
  ...
  <main>
    ...
    <article>
      <h1 id="title">Welcome to ${opts.name}</h1>
      <p id="message">Ready to start the installation. This will only take a moment.</p>

      <button id="installBtn">Install ${opts.name}</button>
      ...
      <div id="status" class="status hidden">
        ...
        <div id="resetGroup" class="reset-group hidden">
          <input
            type="text"
            id="resetInput"
          ...
          <div class="button-group">
            <button id="resetQuitBtn" class="secondary">Quit</button>
            <button id="resetBtn" class="destructive">Reset app</button>
          </div>
        </div>
      </div>
    </article>
  ...
  <script>
    ...
    const elements = {
      ...
      resetGroup: document.getElementById('resetGroup'),
      resetQuitBtn: document.getElementById('resetQuitBtn'),
    ...
    function resetElements(e) {
      ...
      e.warning.classList.remove('error', 'nuclear')
      e.progress.classList.remove('hidden')
      e.buttonGroup.classList.add('hidden')
      e.launchBtn.classList.add('hidden')
      e.resetGroup.classList.add('hidden')
      e.resetInput.value = ''
      e.resetBtn.disabled = false
      e.errorMessage.classList.add('hidden')
      e.errorMessage.textContent = ''
    }

    function setProgress({ speed, peers, progress, stage, bytes, rawBytes }) {
      // Update stats
      ...
      if (progress !== undefined) {
        ...
        if (progress === 0 && !rawBytes) {
          elements.progress.classList.remove('determinate', 'transitioning')
    ...
    function setState(state, error) {
      ...
      switch (state) {
        case 'installing':
          title.textContent = 'Welcome to ${opts.name}'
          message.textContent = '${opts.name} will launch once installed.'
          break
        case 'slow':
          title.textContent = 'Welcome to ${opts.name}'
          message.textContent = '${opts.name} will launch once installed.'
        ...
        case 'error':
          ...
          break
        case 'nuclear':
          title.textContent = 'Looks like ${opts.name} was restored from a backup'
          message.textContent =
            'To proceed you need to clear that backup first. You can relink your device afterward to access your data. This will reset Pear on this device.'
          warning.innerHTML =
            'To automatically do this type <strong>' +
            RESET_CONFIRMATION +
            '</strong> in the input bellow.'
          warning.classList.remove('hidden')
          warning.classList.remove('error')
          warning.classList.add('nuclear')
          elements.status.classList.remove('hidden')
          elements.progress.classList.add('hidden')
          elements.resetGroup.classList.remove('hidden')
          setTimeout(() => elements.resetInput.focus(), 100)
          break
        case 'complete':
          title.textContent = 'Installation complete!'
          progress.classList.add('complete')
          if (AUTO_LAUNCH) {
            message.textContent = '${opts.name} app will launch shortly'
            setTimeout(() => bridge.postMessage('launch'), 500)
          } else {
            message.textContent = '${opts.name} is ready to launch.'
    ...
    onMessage(bridge, (event) => {
      const { type, state, data, error } = event.data || {}
      switch (type) {
        case 'state':
    ...
    elements.resetInput.addEventListener('input', (e) => {
      const value = e.target.value.trim()
      elements.resetBtn.disabled = value !== RESET_CONFIRMATION
    })

    onClick(elements.resetBtn, () => {
      if (elements.resetInput.value.trim() === RESET_CONFIRMATION) {
        setState('installing')
        bridge.postMessage('reset')
        startTimer()
      }
    })

    onClick(elements.resetQuitBtn, () => {
      bridge.postMessage('quit')
    })
  </script>
`
